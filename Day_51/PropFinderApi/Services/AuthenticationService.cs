...
namespace PropFinderApi.Services
{
    public class AuthenticationService : IAuthenticationService
    {
        ...
        {
            var existingUser = await _userService.GetUserByEmailandRoleAsync(registerRequest.Email, registerRequest.Role);

            if (existingUser != null)
                throw new ConflictException("An account with this email and role already exists.");
        ...
        {
            var user = await _userService.GetUserByEmailandRoleAsync(loginRequest.Email, loginRequest.Role ?? Models.Enums.UserRole.Admin);

            if (user == null)
                throw new UnauthorizedException("Invalid email or password.");

            if (user.IsDeleted)
                throw new NotFoundException("User has been disabled.");

            if(!_encryptionService.VerifyPassword(loginRequest.Password, user.PasswordHash))
                throw new UnauthorizedException("Invalid email or password.");
