...
SELECT title, pub_id, type, price FROM titles
ORDER BY pub_id DESC, type ASC, price DESC;
 
-- 9) Print the average price of books in every type

SELECT type, AVG(price) AS Average_Price FROM titles
GROUP BY type;
...
SELECT CONCAT(au_fname, ' ', au_lname) as AuthorName FROM authors
WHERE state = 'CA';
 
-- 15) Print the count of authors from every state

SELECT state, COUNT(*) AS Authors_Count FROM authors
GROUP BY state; 
