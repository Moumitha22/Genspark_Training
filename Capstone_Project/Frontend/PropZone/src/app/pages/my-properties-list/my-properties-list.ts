...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, PropertyCard, CoreFiltersComponent, RouterLink, FormsModule],
...
export class MyPropertiesListComponent implements OnInit {
  ...
  applyFiltersAndSorting(): void {
    ...
    let result = this.allProperties.filter(p => {
      const matchPurpose = !filters.listingPurpose || filters.listingPurpose == p.listingPurpose;
      const matchType = !filters.propertyTypes || filters.propertyTypes.length === 0 || filters.propertyTypes.includes(p.propertyType);
      const matchLister = !filters.listerTypes || filters.listerTypes.length === 0 || filters.listerTypes.includes(p.listerType);
      const matchStatus = !filters.status || p.status === filters.status;
      const matchCity = !filters.city || p.location.city?.toLowerCase().includes(filters.city.toLowerCase());
      ...
      return matchPurpose && matchType && matchLister && matchStatus && matchCity && matchKeyword;
    });
