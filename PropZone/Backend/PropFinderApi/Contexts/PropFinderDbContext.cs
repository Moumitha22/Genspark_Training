...
namespace PropFinderApi.Contexts
{
    ...
    {
        public PropFinderDbContext(DbContextOptions<PropFinderDbContext> options)
            : base(options) { }
        ...
        public DbSet<DiscountCodeOptions> DiscountCodeOptions { get; set; }
        public DbSet<PropertyDiscountCode> PropertyDiscountCodes { get; set; }
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            {
                ListerProfile.HasKey(lp => lp.Id).HasName("PK_ListerProfile");
            });

            // Property
            modelBuilder.Entity<Property>(property =>
            {
                property.HasKey(p => p.Id).HasName("PK_Property");

                property.Property(p => p.PropertyType).HasConversion<string>();

                property.Property(p => p.ListingPurpose).HasConversion<string>();

                property.Property(p => p.ListerType).HasConversion<string>();

                property.Property(p => p.Status).HasConversion<string>();
            ...
            {
                PropertyLocation.HasKey(pl => pl.Id).HasName("PK_PropertyLocation");
            });

            // PropertyImage
            modelBuilder.Entity<PropertyImage>(propertyImage =>
            {
                propertyImage.HasKey(pi => pi.Id).HasName("PK_PropertyImage");
            });

            // ContactLog
            modelBuilder.Entity<ContactLog>(contactLog =>
            {
                contactLog.HasKey(cl => cl.Id).HasName("PK_ContactLog");
            });

            // RefreshToken
            modelBuilder.Entity<RefreshToken>(refreshToken =>
            {
                refreshToken.HasKey(rt => rt.Id).HasName("PK_RefreshToken");
            ...
            {
                featureMaster.HasKey(fm => fm.Id).HasName("PK_FeatureMaster");

                featureMaster.Property(fm => fm.DataType).HasConversion<string>();
            });

            // FEATURE OPTION
            modelBuilder.Entity<FeatureOption>(featureOption =>
            {
                featureOption.HasKey(fo => fo.Id).HasName("PK_FeatureOption");

                featureOption
                    .HasOne(fo => fo.Feature)
            ...
            modelBuilder.Entity<FeatureApplicability>(featureApplicability =>
            {
                ...
                featureApplicability.Property(fa => fa.AppliesToType).HasConversion<string>();

                featureApplicability.Property(fa => fa.AppliesToPurpose).HasConversion<string>();
            ...
            {
                propertyFeature.HasKey(pf => pf.Id).HasName("PK_PropertyFeature");
            ...
            // {
            //     entity.Property(dc => dc.CreatedAt).HasDefaultValueSql("now()");
            //     entity.Property(dc => dc.UpdatedAt).HasDefaultValueSql("now()");
            // });
            modelBuilder.Entity<DiscountCodeOptions>(discountCode =>
            {
                discountCode.HasKey(dc => dc.Id).HasName("PK_DiscountCode");
                discountCode.Property(dc => dc.TypeOfProperty).HasConversion<string>();
                discountCode.Property(dc => dc.PurposeOfListing).HasConversion<string>();
            });

            modelBuilder.Entity<PropertyDiscountCode>(entity =>
            {
                entity
                    .HasKey(pd => new { pd.PropertyId, pd.DiscountCodeId })
                    .HasName("PK_PropertyDiscountCode");

                entity
                    .HasOne(pd => pd.Property)
                    .WithMany(p => p.PropertyDiscountCodes)
                    .HasForeignKey(pd => pd.PropertyId);

                entity
                    .HasOne(pd => pd.DiscountCode)
                    .WithMany(dc => dc.PropertyDiscountCodes)
                    .HasForeignKey(pd => pd.DiscountCodeId);
            });
        }
    }
}
