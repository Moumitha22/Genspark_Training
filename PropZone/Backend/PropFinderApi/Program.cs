...
var isEfTool = AppDomain.CurrentDomain.FriendlyName.ToLower().Contains("ef");
try
{
                ...
                {
                    {
                        ...
                        new[]
                        {
                            "You have exceeded the allowed number of requests. Try again shortly.",
                        }
                    },
                },
            };
    ...
    builder.Services.AddTransient<IListerProfileRepository, ListerProfileRepository>();
    builder.Services.AddTransient<IPropertyRepository, PropertyRepository>();
    builder.Services.AddTransient<IPropertyLocationRepository, PropertyLocationRepository>();
    builder.Services.AddTransient<IPropertyFeatureRepository, PropertyFeatureRepository>();
    builder.Services.AddTransient<IFeatureMasterRepository, FeatureMasterRepository>();
    ...
    builder.Services.AddTransient<IFeatureMasterService, FeatureMasterService>();
    builder.Services.AddTransient<IPropertyImageService, PropertyImageService>();
    builder.Services.AddTransient<IListerProfileService, ListerProfileService>();
            ...
            options.Events = new JwtBearerEvents
            {
                ...
                OnChallenge = context =>
                {
                    var userId =
                        ...
                        ?? "Anonymous";
                    var email =
                        context.HttpContext.User.FindFirst(ClaimTypes.Email)?.Value ?? "Unknown";

                    Log.Warning(
                        "401 Unauthorized access attempt by {UserId} ({Email}) to {Path}",
                        userId,
                        email,
                    ...
                    var response = new
                    {
                        ...
                        errors = new Dictionary<string, string[]>
                        {
                            {
                                "general",
                                new[] { "Authentication failed or token is missing/invalid." }
                            },
                        },
                    };
                ...
                OnForbidden = context =>
                {
                    ...
                    Log.Warning(
                        "403 Forbidden: {UserId} ({Email}) attempted to access {Path} without sufficient permissions.",
                        userId,
                        email,
                        context.Request.Path
                    );
