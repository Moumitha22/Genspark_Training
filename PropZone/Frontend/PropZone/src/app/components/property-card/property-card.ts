...
export class PropertyCard {
  ...
  @Input() isDicountClickable: boolean = false;
  @Input() discountedPrice?: number;
  @Output() compareClicked = new EventEmitter<PropertyModel>();
  @Output() simulateDiscount = new EventEmitter<DiscountSimulationRequest>();

  @Input() selectedDiscountIds: string[] = [];
  ...
  get amenitiesFeature() {
    return this.property.featureSummary?.find(
      (f) => f.featureName === 'Amenities'
    );
  }
  ...
  onContactClick() {
    this.contactClicked.emit({
      ...
      location: `${this.property.location.locality}, ${this.property.location.city}, ${this.property.location.state}`,
    });
  }

  toggleDiscountCode(discount: DiscountCode) {
    if(!this.isDicountClickable){
      return;
    }
    if(this.discountedPrice && this.discountedPrice >= this.property.price){
      this.selectedDiscountIds.push(discount.id);

    }
    const id = this.selectedDiscountIds.indexOf(discount.id);
    if (id > -1) {
      this.selectedDiscountIds.splice(id, 1);
    } else {
      this.selectedDiscountIds.push(discount.id);
    }
    const discountCodeIds = this.selectedDiscountIds;
    const price = this.property.price;
    const simulateRequest: DiscountSimulationRequest = {
      discountCodeIds,
      price,
    };
    this.simulateDiscount.emit(simulateRequest);
  }
}
